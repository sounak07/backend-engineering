...
- *Advantages:* - Minimizes disruption to existing services. - Allows for a gradual transition to microservices. 
***Example**:* 
...
[2 Phase commit and Saga](https://www.youtube.com/watch?v=DOFflggE_0Q)
...
In case the orchestrator crashes, it reads from its durable state and proceeds the compensating actions or next steps. No locks dangling like 2P. 

If with Saga, there could be issues. 
- Compensating actions keep log traces, notifications unless db transactions
- Actions like notifications, sending email, calling a hook can't be compensated in a similar manner. 
- Strong idempotency to carry out retries in case compensating payments fail. 
...
2. Publish an **event/message** (e.g., to Kafka, RabbitMQ)

…these two actions are often done separately.

👉 If one succeeds and the other fails, you get inconsistencies:

- DB updated but no event sent → other services stay outdated
- Event sent but DB not updated → consumers act on invalid data


💡 The Idea Behind the Pattern

Instead of sending messages directly, you:

1. **Write the business data AND the event into the same database transaction**
2. Store the event in a special table called the **outbox**
3. A separate process (CDC) reads from the outbox and publishes messages
...
- *Example:* - A blog application where write operations are performed on a relational database, while read operations access a denormalized view optimized
